...
function s:enable_lsp_mappings() abort
    ...
    nmap <buffer> <expr><esc><c-n>  lsp#scroll(+4)
    nmap <buffer> <expr><esc><c-p>  lsp#scroll(-4)
    nmap <buffer> <C-]>             :LspDefinition<cr>
    nmap <buffer> gh                :LspReferences<cr>
    nmap <buffer> gp                :LspDocumentFormat<cr>
    nmap <buffer> <silent><leader>d :call Custom_vim_lsp_show_diagnostic()<cr>
    nmap <buffer> <silent>]d        :LspPreviousDiagnostic<cr>gK
    nmap <buffer> <silent>[d        :LspNextDiagnostic<cr>gK
    nmap <buffer> <silent>]a        :LspDocumentDiagnostics<cr>
    nmap <buffer> g.                :LspCodeAction<cr>
    imap <buffer> <C-i>             <cmd>LspSignatureHelp<cr>
    imap <buffer> <C-@>             <Plug>(asyncomplete_force_refresh)
